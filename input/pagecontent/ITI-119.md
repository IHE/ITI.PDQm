...
A Resource Bundle is returned representing the zero result set. No `entry` attributes are provided in the result. The Patient Demographics Supplier MAY populate the total element of the bundle with 0 to indicate no matching results. 
...
A Resource Bundle is returned representing the zero result set. No `entry` attributes are provided in the result. [Example output](Bundle-ex-match-output-empty.html). The Patient Demographics Supplier MAY populate the total element of the bundle with 0 to indicate no matching results. 
...
OperationOutcome Resources returned do not have `error` or `fatal` severity. [Example output](Bundle-ex-match-output-empty.html). The Patient Demographics Supplier MAY populate the total element of the bundle with the number of returned matching results. 
