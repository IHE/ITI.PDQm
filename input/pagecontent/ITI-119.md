...
**Case 1**: The Patient Demographics Supplier finds at least one patient record matching the criteria sent as a Patient Resource. The `onlyCertainMatches` parameter is not set to `true`.
...
**Case 3**: The Patient Demographics Supplier fails to find in its information source, any patient record matching the criteria sent as a Patient Resource.
